...
describe('BN.js/Reduction context', function () {
  function testMethod (name: string, fn: (num: IPrimeName | BigNumber) => Red) {
    describe(name + ' method', function () {
      it('should support add, iadd, sub, isub operations', function () {
        var p = new BigNumber(257);
        var m = fn(p);
        var a = new BigNumber(123).toRed(m);
        var b = new BigNumber(231).toRed(m);
      ...
      it('should support pow and mul operations', function () {
        var p192 = new BigNumber(
          'fffffffffffffffffffffffffffffffeffffffffffffffff',
          16);
        var m = fn(p192);
        var a = new BigNumber(123);
        var b = new BigNumber(231);
      ...
      it('should sqrtm numbers', function () {
        var p = new BigNumber(263);
        var m = fn(p);
        var q = new BigNumber(11).toRed(m);
      ...
      it('should invm numbers (regression)', function () {
        ...
        assert.equal(aa.redInvm().fromRed().negative, 0);
      });
      ...
      it('should imul numbers', function () {
        ...
        var a = new BigNumber('deadbeefabbadead', 16);
        var b = new BigNumber('abbadeadbeefdead', 16);
        var c = a.mul(b).mod(p);
      ...
      it('should pow(base, 0) == 1', function () {
        var base = new BigNumber(256).toRed(BigNumber.red('k256'));
        var exponent = new BigNumber(0);
      ...
      it('should reduce when converting to red', function () {
        var p = new BigNumber(257);
        var m = fn(p);
        var a = new BigNumber(5).toRed(m);

        assert.doesNotThrow(function () {
          var b = a.redISub(new BigNumber(512).toRed(m));
          b.redISub(new BigNumber(512).toRed(m));
        });
      });

      it('redNeg and zero value', function () {
        var a = new BigNumber(0).toRed(BigNumber.red('k256')).redNeg();
  ...
  describe('Pseudo-Mersenne Primes', function () {
    ...
    it('redISqr should return right result', function () {
      var n = new BigNumber('30f28939', 16);
      var actual = n.toRed(BigNumber.red('k256')).redISqr().fromRed();
      assert.equal(actual.toString(16), '95bd93d19520eb1');
    });
  });

  it('should avoid 4.1.0 regresion', function () {
    function bits2int (obits: any, q: any) {
      var bits = new BigNumber(obits);
    ...
    var p = new BigNumber(
      ...
      16);
    var q = new BigNumber('f2c3119374ce76c9356990b465374a17f23f9ed35089bd969f61c6dde' +
      '9998c1f', 16);
    var k = bits2int(t, q);
  ...
  it('K256.split for 512 bits number should return equal numbers', function () {
    var red = BigNumber.red('k256');
    var input = new BigNumber(1).iushln(512).subn(1);
    assert.equal(input.bitLength(), 512);
    var output = new BigNumber(0);
    red["prime"]!.split(input, output);
    assert.equal(input.cmp(output), 0);
  });

  it('imod should change host object', function () {
    var red = BigNumber.red(new BigNumber(13));
    var a = new BigNumber(2).toRed(red);
    var b = new BigNumber(7).toRed(red);
    var c = a.redIMul(b);
