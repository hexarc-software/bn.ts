...
describe("BN.ts/Arithmetic", () => {
  ...
  function testMethod (name: string, mul: (left: BN, right: BN) => BN) {
    describe(name, () => {
  ...
  testMethod(".mul()", (x: BN, y: BN) => {
    return BN.prototype.mul.apply(x, [y]);
  });

  testMethod(".mulf()", (x: BN, y: BN) => {
  ...
  describe(".egcd()", () => {
    ...
    it("should not allow 0 input", () => {
      assert.throws(() => {
        new BN(1).egcd(0 as any);
      }, /^Error: Assertion failed$/);
    });
    it("should not allow negative input", () => {
      assert.throws(() => {
        new BN(1).egcd(-1 as any);
      }, /^Error: Assertion failed$/);
    });
  });
