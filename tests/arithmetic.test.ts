import { strict as assert } from "assert";
import { BigNumber } from "../src/bn";
import * as fixtures from "./fixtures";

describe("BN.js/Arithmetic", () => {
  describe(".add()", () => {
    it("should add numbers", () => {
      assert.equal(new BigNumber(14).add(new BigNumber(26)).toString(16), "28");
      const k = new BigNumber(0x1234);
      let r = k;

      for (let i = 0; i < 257; i++) {
        r = r.add(k);
      }

      assert.equal(r.toString(16), "125868");
    });

    it("should handle carry properly (in-place)", () => {
      const k = new BigNumber("abcdefabcdefabcdef", 16);
      const r = new BigNumber("deadbeef", 16);

      for (let i = 0; i < 257; i++) {
        r.iadd(k);
      }

      assert.equal(r.toString(16), "ac79bd9b79be7a277bde");
    });

    it("should properly do positive + negative", () => {
      let a = new BigNumber("abcd", 16);
      let b = new BigNumber("-abce", 16);

      assert.equal(a.iadd(b).toString(16), "-1");

      a = new BigNumber("abcd", 16);
      b = new BigNumber("-abce", 16);

      assert.equal(a.add(b).toString(16), "-1");
      assert.equal(b.add(a).toString(16), "-1");
    });
  });

  describe(".iaddn()", () => {
    it("should allow a sign change", () => {
      const a = new BigNumber(-100);
      assert.equal(a.negative, 1);

      a.iaddn(200);

      assert.equal(a.negative, 0);
      assert.equal(a.toString(), "100");
    });

    it("should add negative number", () => {
      const a = new BigNumber(-100);
      assert.equal(a.negative, 1);

      a.iaddn(-200);

      assert.equal(a.toString(), "-300");
    });

    it("should allow neg + pos with big number", () => {
      const a = new BigNumber("-1000000000", 10);
      assert.equal(a.negative, 1);

      a.iaddn(200);

      assert.equal(a.toString(), "-999999800");
    });

    it("should carry limb", () => {
      const a = new BigNumber("3ffffff", 16);

      assert.equal(a.iaddn(1).toString(16), "4000000");
    });

    it("should throw error with num eq 0x4000000", () => {
      assert.throws(() => {
        new BigNumber(0).iaddn(0x4000000);
      }, /^Error: Assertion failed$/);
    });

    it("should reset sign if value equal to value in instance", () => {
      const a = new BigNumber(-1);
      assert.equal(a.addn(1).toString(), "0");
    });
  });

  describe(".sub()", () => {
    it("should subtract small numbers", () => {
      assert.equal(new BigNumber(26).sub(new BigNumber(14)).toString(16), "c");
      assert.equal(new BigNumber(14).sub(new BigNumber(26)).toString(16), "-c");
      assert.equal(new BigNumber(26).sub(new BigNumber(26)).toString(16), "0");
      assert.equal(new BigNumber(-26).sub(new BigNumber(26)).toString(16), "-34");
    });

    const a = new BigNumber(
      "31ff3c61db2db84b9823d320907a573f6ad37c437abe458b1802cda041d6384" +
      "a7d8daef41395491e2",
      16);
    const b = new BigNumber(
      "6f0e4d9f1d6071c183677f601af9305721c91d31b0bbbae8fb790000",
      16);
    const r = new BigNumber(
      "31ff3c61db2db84b9823d3208989726578fd75276287cd9516533a9acfb9a67" +
      "76281f34583ddb91e2",
      16);

    it("should subtract big numbers", () => {
      assert.equal(a.sub(b).cmp(r), 0);
    });

    it("should subtract numbers in place", () => {
      assert.equal(b.clone().isub(a).neg().cmp(r), 0);
    });

    it("should subtract with carry", () => {
      // Carry and copy
      let a = new BigNumber("12345", 16);
      let b = new BigNumber("1000000000000", 16);
      assert.equal(a.isub(b).toString(16), "-fffffffedcbb");

      a = new BigNumber("12345", 16);
      b = new BigNumber("1000000000000", 16);
      assert.equal(b.isub(a).toString(16), "fffffffedcbb");
    });
  });

  describe(".isuBigNumber()", () => {
    it("should subtract negative number", () => {
      const r = new BigNumber(
        "7fffffffffffffffffffffffffffffff5d576e7357a4501ddfe92f46681b", 16);
      assert.equal(r.isubn(-1).toString(16),
        "7fffffffffffffffffffffffffffffff5d576e7357a4501ddfe92f46681c");
    });

    it("should work for positive numbers", () => {
      const a = new BigNumber(-100);
      assert.equal(a.negative, 1);

      a.isubn(200);
      assert.equal(a.negative, 1);
      assert.equal(a.toString(), "-300");
    });

    it("should not allow a sign change", () => {
      const a = new BigNumber(-100);
      assert.equal(a.negative, 1);

      a.isubn(-200);
      assert.equal(a.negative, 0);
      assert.equal(a.toString(), "100");
    });

    it("should change sign on small numbers at 0", () => {
      const a = new BigNumber(0).isubn(2);
      assert.equal(a.toString(), "-2");
    });

    it("should change sign on small numbers at 1", () => {
      const a = new BigNumber(1).isubn(2);
      assert.equal(a.toString(), "-1");
    });

    it("should throw error with num eq 0x4000000", () => {
      assert.throws(() => {
        new BigNumber(0).isubn(0x4000000);
      }, /^Error: Assertion failed$/);
    });
  });

  function testMethod (name, mul) {
    describe(name, () => {
      it("should multiply numbers of different signs", () => {
        const offsets = [
          1, // smallMulTo
          250, // comb10MulTo
          1000, // bigMulTo
          15000 // jumboMulTo
        ];

        for (let i = 0; i < offsets.length; ++i) {
          const x = new BigNumber(1).ishln(offsets[i]);

          assert.equal(mul(x, x).isNeg(), false);
          assert.equal(mul(x, x.neg()).isNeg(), true);
          assert.equal(mul(x.neg(), x).isNeg(), true);
          assert.equal(mul(x.neg(), x.neg()).isNeg(), false);
        }
      });

      it("should multiply with carry", () => {
        const n = new BigNumber(0x1001);
        let r = n;

        for (let i = 0; i < 4; i++) {
          r = mul(r, n);
        }

        assert.equal(r.toString(16), "100500a00a005001");
      });

      it("should correctly multiply big numbers", () => {
        const n = new BigNumber(
          "79be667ef9dcbbac55a06295ce870b07029bfcdb2dce28d959f2815b16f81798",
          16
        );
        assert.equal(
          mul(n, n).toString(16),
          "39e58a8055b6fb264b75ec8c646509784204ac15a8c24e05babc9729ab9" +
          "b055c3a9458e4ce3289560a38e08ba8175a9446ce14e608245ab3a9" +
          "978a8bd8acaa40");
        assert.equal(
          mul(mul(n, n), n).toString(16),
          "1b888e01a06e974017a28a5b4da436169761c9730b7aeedf75fc60f687b" +
          "46e0cf2cb11667f795d5569482640fe5f628939467a01a612b02350" +
          "0d0161e9730279a7561043af6197798e41b7432458463e64fa81158" +
          "907322dc330562697d0d600");
      });

      it("should multiply neg number on 0", () => {
        assert.equal(
          mul(new BigNumber("-100000000000"), new BigNumber("3").div(new BigNumber("4")))
            .toString(16),
          "0"
        );
      });

      it("should regress mul big numbers", () => {
        const q = fixtures.dhGroups.p17.q;
        const qs = fixtures.dhGroups.p17.qs;

        let qn = new BigNumber(q, 16);
        assert.equal(mul(qn, qn).toString(16), qs);
      });
    });
  }

  testMethod(".mul()", function (x, y) {
    return BigNumber.prototype.mul.apply(x, [y]);
  });

  testMethod(".mulf()", function (x, y) {
    return BigNumber.prototype.mulf.apply(x, [y]);
  });

  describe(".imul()", () => {
    it("should multiply numbers in-place", () => {
      let a = new BigNumber("abcdef01234567890abcd", 16);
      let b = new BigNumber("deadbeefa551edebabba8", 16);
      let c = a.mul(b);

      assert.equal(a.imul(b).toString(16), c.toString(16));

      a = new BigNumber("abcdef01234567890abcd214a25123f512361e6d236", 16);
      b = new BigNumber("deadbeefa551edebabba8121234fd21bac0341324dd", 16);
      c = a.mul(b);

      assert.equal(a.imul(b).toString(16), c.toString(16));
    });

    it("should multiply by 0", () => {
      const a = new BigNumber("abcdef01234567890abcd", 16);
      const b = new BigNumber("0", 16);
      const c = a.mul(b);

      assert.equal(a.imul(b).toString(16), c.toString(16));
    });

    it("should regress mul big numbers in-place", () => {
      const q = fixtures.dhGroups.p17.q;
      const qs = fixtures.dhGroups.p17.qs;

      let qr = new BigNumber(q, 16);

      assert.equal(qr.isqr().toString(16), qs);
    });
  });

  describe(".muln()", () => {
    it("should multiply number by small number", () => {
      const a = new BigNumber("abcdef01234567890abcd", 16);
      const b = new BigNumber("dead", 16);
      const c = a.mul(b);

      assert.equal(a.muln(0xdead).toString(16), c.toString(16));
    });

    it("should throw error with num eq 0x4000000", () => {
      assert.throws(() => {
        new BigNumber(0).imuln(0x4000000);
      }, /^Error: Assertion failed$/);
    });

    it("should negate number if number is negative", () => {
      const a = new BigNumber("dead", 16);
      assert.equal(a.clone().imuln(-1).toString(16), a.clone().neg().toString(16));
      assert.equal(a.clone().muln(-1).toString(16), a.clone().neg().toString(16));

      const b = new BigNumber("dead", 16);
      assert.equal(b.clone().imuln(-42).toString(16), b.clone().neg().muln(42).toString(16));
      assert.equal(b.clone().muln(-42).toString(16), b.clone().neg().muln(42).toString(16));
    });
  });

  describe(".pow()", () => {
    it("should raise number to the power", () => {
      const a = new BigNumber("ab", 16);
      const b = new BigNumber("13", 10);
      const c = a.pow(b);

      assert.equal(c.toString(16), "15963da06977df51909c9ba5b");
    });
  });

  describe(".div()", () => {
    it("should divide small numbers (<=26 bits)", () => {
      assert.equal(new BigNumber("256").div(new BigNumber(10)).toString(10),
        "25");
      assert.equal(new BigNumber("-256").div(new BigNumber(10)).toString(10),
        "-25");
      assert.equal(new BigNumber("256").div(new BigNumber(-10)).toString(10),
        "-25");
      assert.equal(new BigNumber("-256").div(new BigNumber(-10)).toString(10),
        "25");

      assert.equal(new BigNumber("10").div(new BigNumber(256)).toString(10),
        "0");
      assert.equal(new BigNumber("-10").div(new BigNumber(256)).toString(10),
        "0");
      assert.equal(new BigNumber("10").div(new BigNumber(-256)).toString(10),
        "0");
      assert.equal(new BigNumber("-10").div(new BigNumber(-256)).toString(10),
        "0");
    });

    it("should divide large numbers (>53 bits)", () => {
      assert.equal(new BigNumber("1222222225255589").div(new BigNumber("611111124969028"))
        .toString(10), "1");
      assert.equal(new BigNumber("-1222222225255589").div(new BigNumber("611111124969028"))
        .toString(10), "-1");
      assert.equal(new BigNumber("1222222225255589").div(new BigNumber("-611111124969028"))
        .toString(10), "-1");
      assert.equal(new BigNumber("-1222222225255589").div(new BigNumber("-611111124969028"))
        .toString(10), "1");

      assert.equal(new BigNumber("611111124969028").div(new BigNumber("1222222225255589"))
        .toString(10), "0");
      assert.equal(new BigNumber("-611111124969028").div(new BigNumber("1222222225255589"))
        .toString(10), "0");
      assert.equal(new BigNumber("611111124969028").div(new BigNumber("-1222222225255589"))
        .toString(10), "0");
      assert.equal(new BigNumber("-611111124969028").div(new BigNumber("-1222222225255589"))
        .toString(10), "0");
    });

    it("should divide numbers", () => {
      assert.equal(new BigNumber("69527932928").div(new BigNumber("16974594")).toString(16),
        "fff");
      assert.equal(new BigNumber("-69527932928").div(new BigNumber("16974594")).toString(16),
        "-fff");

      const b = new BigNumber(
        "39e58a8055b6fb264b75ec8c646509784204ac15a8c24e05babc9729ab9" +
        "b055c3a9458e4ce3289560a38e08ba8175a9446ce14e608245ab3a9" +
        "978a8bd8acaa40",
        16);
      const n = new BigNumber(
        "79be667ef9dcbbac55a06295ce870b07029bfcdb2dce28d959f2815b16f81798",
        16
      );
      assert.equal(b.div(n).toString(16), n.toString(16));

      assert.equal(new BigNumber("1").div(new BigNumber("-5")).toString(10), "0");
    });

    it("should not fail on regression after moving to _wordDiv", () => {
      // Regression after moving to word div
      let p = new BigNumber(
        "fffffffffffffffffffffffffffffffffffffffffffffffffffffffefffffc2f",
        16);
      let a = new BigNumber(
        "79be667ef9dcbbac55a06295ce870b07029bfcdb2dce28d959f2815b16f81798",
        16);
      const as = a.sqr();
      assert.equal(
        as.div(p).toString(16),
        "39e58a8055b6fb264b75ec8c646509784204ac15a8c24e05babc9729e58090b9");

      p = new BigNumber(
        "ffffffff00000001000000000000000000000000ffffffffffffffffffffffff",
        16);
      a = new BigNumber(
        "fffffffe00000003fffffffd0000000200000001fffffffe00000002ffffffff" +
        "ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff",
        16);
      assert.equal(
        a.div(p).toString(16),
        "ffffffff00000002000000000000000000000001000000000000000000000001");
    });
  });

  describe(".idivn()", () => {
    it("should divide numbers in-place", () => {
      assert.equal(new BigNumber("10", 16).idivn(3).toString(16), "5");
      assert.equal(new BigNumber("10", 16).idivn(-3).toString(16), "-5");
      assert.equal(new BigNumber("12", 16).idivn(3).toString(16), "6");
      assert.equal(new BigNumber("10000000000000000").idivn(3).toString(10),
        "3333333333333333");
      assert.equal(
        new BigNumber("100000000000000000000000000000").idivn(3).toString(10),
        "33333333333333333333333333333");

      const t = new BigNumber(3);
      assert.equal(
        new BigNumber("12345678901234567890123456", 16).idivn(3).toString(16),
        new BigNumber("12345678901234567890123456", 16).div(t).toString(16));
    });
  });

  describe(".divRound()", () => {
    it("should divide numbers with rounding", () => {
      assert.equal(new BigNumber(9).divRound(new BigNumber(20)).toString(10),
        "0");
      assert.equal(new BigNumber(10).divRound(new BigNumber(20)).toString(10),
        "1");
      assert.equal(new BigNumber(150).divRound(new BigNumber(20)).toString(10),
        "8");
      assert.equal(new BigNumber(149).divRound(new BigNumber(20)).toString(10),
        "7");
      assert.equal(new BigNumber(149).divRound(new BigNumber(17)).toString(10),
        "9");
      assert.equal(new BigNumber(144).divRound(new BigNumber(17)).toString(10),
        "8");
      assert.equal(new BigNumber(-144).divRound(new BigNumber(17)).toString(10),
        "-8");
    });

    it("should return 1 on exact division", () => {
      assert.equal(new BigNumber(144).divRound(new BigNumber(144)).toString(10), "1");
    });
  });

  describe(".mod()", () => {
    it("should modulo small numbers (<=26 bits)", () => {
      assert.equal(new BigNumber("256").mod(new BigNumber(10)).toString(10),
        "6");
      assert.equal(new BigNumber("-256").mod(new BigNumber(10)).toString(10),
        "-6");
      assert.equal(new BigNumber("256").mod(new BigNumber(-10)).toString(10),
        "6");
      assert.equal(new BigNumber("-256").mod(new BigNumber(-10)).toString(10),
        "-6");

      assert.equal(new BigNumber("10").mod(new BigNumber(256)).toString(10),
        "10");
      assert.equal(new BigNumber("-10").mod(new BigNumber(256)).toString(10),
        "-10");
      assert.equal(new BigNumber("10").mod(new BigNumber(-256)).toString(10),
        "10");
      assert.equal(new BigNumber("-10").mod(new BigNumber(-256)).toString(10),
        "-10");
    });

    it("should modulo large numbers (>53 bits)", () => {
      assert.equal(new BigNumber("1222222225255589").mod(new BigNumber("611111124969028"))
        .toString(10), "611111100286561");
      assert.equal(new BigNumber("-1222222225255589").mod(new BigNumber("611111124969028"))
        .toString(10), "-611111100286561");
      assert.equal(new BigNumber("1222222225255589").mod(new BigNumber("-611111124969028"))
        .toString(10), "611111100286561");
      assert.equal(new BigNumber("-1222222225255589").mod(new BigNumber("-611111124969028"))
        .toString(10), "-611111100286561");

      assert.equal(new BigNumber("611111124969028").mod(new BigNumber("1222222225255589"))
        .toString(10), "611111124969028");
      assert.equal(new BigNumber("-611111124969028").mod(new BigNumber("1222222225255589"))
        .toString(10), "-611111124969028");
      assert.equal(new BigNumber("611111124969028").mod(new BigNumber("-1222222225255589"))
        .toString(10), "611111124969028");
      assert.equal(new BigNumber("-611111124969028").mod(new BigNumber("-1222222225255589"))
        .toString(10), "-611111124969028");
    });

    it("should mod numbers", () => {
      assert.equal(new BigNumber("10").mod(new BigNumber(256)).toString(16),
        "a");
      assert.equal(new BigNumber("69527932928").mod(new BigNumber("16974594")).toString(16),
        "102f302");

      // 178 = 10 * 17 + 8
      assert.equal(new BigNumber(178).div(new BigNumber(10)).toNumber(), 17);
      assert.equal(new BigNumber(178).mod(new BigNumber(10)).toNumber(), 8);
      assert.equal(new BigNumber(178).umod(new BigNumber(10)).toNumber(), 8);

      // -178 = 10 * (-17) + (-8)
      assert.equal(new BigNumber(-178).div(new BigNumber(10)).toNumber(), -17);
      assert.equal(new BigNumber(-178).mod(new BigNumber(10)).toNumber(), -8);
      assert.equal(new BigNumber(-178).umod(new BigNumber(10)).toNumber(), 2);

      // 178 = -10 * (-17) + 8
      assert.equal(new BigNumber(178).div(new BigNumber(-10)).toNumber(), -17);
      assert.equal(new BigNumber(178).mod(new BigNumber(-10)).toNumber(), 8);
      assert.equal(new BigNumber(178).umod(new BigNumber(-10)).toNumber(), 8);

      // -178 = -10 * (17) + (-8)
      assert.equal(new BigNumber(-178).div(new BigNumber(-10)).toNumber(), 17);
      assert.equal(new BigNumber(-178).mod(new BigNumber(-10)).toNumber(), -8);
      assert.equal(new BigNumber(-178).umod(new BigNumber(-10)).toNumber(), 2);

      // -4 = 1 * (-3) + -1
      assert.equal(new BigNumber(-4).div(new BigNumber(-3)).toNumber(), 1);
      assert.equal(new BigNumber(-4).mod(new BigNumber(-3)).toNumber(), -1);

      // -4 = -1 * (3) + -1
      assert.equal(new BigNumber(-4).mod(new BigNumber(3)).toNumber(), -1);
      // -4 = 1 * (-3) + (-1 + 3)
      assert.equal(new BigNumber(-4).umod(new BigNumber(-3)).toNumber(), 2);

      const p = new BigNumber(
        "ffffffff00000001000000000000000000000000ffffffffffffffffffffffff",
        16);
      const a = new BigNumber(
        "fffffffe00000003fffffffd0000000200000001fffffffe00000002ffffffff" +
        "ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff",
        16);
      assert.equal(
        a.mod(p).toString(16),
        "0");
    });

    it("should properly carry the sign inside division", () => {
      const a = new BigNumber("945304eb96065b2a98b57a48a06ae28d285a71b5", "hex");
      const b = new BigNumber(
        "fffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffe",
        "hex");

      assert.equal(a.mul(b).mod(a).cmpn(0), 0);
    });
  });

  describe(".modrn()", () => {
    it("should act like .mod() on small numbers", () => {
      assert.equal(new BigNumber("10", 16).modrn(256).toString(16), "10");
      assert.equal(new BigNumber("10", 16).modrn(-256).toString(16), "-10");
      assert.equal(new BigNumber("100", 16).modrn(256).toString(16), "0");
      assert.equal(new BigNumber("1001", 16).modrn(256).toString(16), "1");
      assert.equal(new BigNumber("100000000001", 16).modrn(256).toString(16), "1");
      assert.equal(new BigNumber("100000000001", 16).modrn(257).toString(16),
        new BigNumber("100000000001", 16).mod(new BigNumber(257)).toString(16));
      assert.equal(new BigNumber("123456789012", 16).modrn(3).toString(16),
        new BigNumber("123456789012", 16).mod(new BigNumber(3)).toString(16));
    });
  });

  describe(".abs()", () => {
    it("should return absolute value", () => {
      assert.equal(new BigNumber(0x1001).abs().toString(), "4097");
      assert.equal(new BigNumber(-0x1001).abs().toString(), "4097");
      assert.equal(new BigNumber("ffffffff", 16).abs().toString(), "4294967295");
    });
  });

  describe(".invm()", () => {
    it("should invert relatively-prime numbers", () => {
      const p = new BigNumber(257);
      let a = new BigNumber(3);
      let b = a.invm(p);
      assert.equal(a.mul(b).mod(p).toString(16), "1");

      const p192 = new BigNumber(
        "fffffffffffffffffffffffffffffffeffffffffffffffff",
        16);
      a = new BigNumber("deadbeef", 16);
      b = a.invm(p192);
      assert.equal(a.mul(b).mod(p192).toString(16), "1");

      // Even base
      const phi = new BigNumber("872d9b030ba368706b68932cf07a0e0c", 16);
      const e = new BigNumber(65537);
      const d = e.invm(phi);
      assert.equal(e.mul(d).mod(phi).toString(16), "1");

      // Even base (take #2)
      a = new BigNumber("5");
      b = new BigNumber("6");
      const r = a.invm(b);
      assert.equal(r.mul(a).mod(b).toString(16), "1");
    });
  });

  describe(".gcd()", () => {
    it("should return GCD", () => {
      assert.equal(new BigNumber(3).gcd(new BigNumber(2)).toString(10), "1");
      assert.equal(new BigNumber(18).gcd(new BigNumber(12)).toString(10), "6");
      assert.equal(new BigNumber(-18).gcd(new BigNumber(12)).toString(10), "6");
      assert.equal(new BigNumber(-18).gcd(new BigNumber(-12)).toString(10), "6");
      assert.equal(new BigNumber(-18).gcd(new BigNumber(0)).toString(10), "18");
      assert.equal(new BigNumber(0).gcd(new BigNumber(-18)).toString(10), "18");
      assert.equal(new BigNumber(2).gcd(new BigNumber(0)).toString(10), "2");
      assert.equal(new BigNumber(0).gcd(new BigNumber(3)).toString(10), "3");
      assert.equal(new BigNumber(0).gcd(new BigNumber(0)).toString(10), "0");
    });
  });

  describe(".egcd()", () => {
    it("should return EGCD", () => {
      assert.equal(new BigNumber(3).egcd(new BigNumber(2)).gcd.toString(10), "1");
      assert.equal(new BigNumber(18).egcd(new BigNumber(12)).gcd.toString(10), "6");
      assert.equal(new BigNumber(-18).egcd(new BigNumber(12)).gcd.toString(10), "6");
      assert.equal(new BigNumber(0).egcd(new BigNumber(12)).gcd.toString(10), "12");
    });
    it("should not allow 0 input", () => {
      assert.throws(() => {
        new BigNumber(1).egcd(0 as any);
      }, /^Error: Assertion failed$/);
    });
    it("should not allow negative input", () => {
      assert.throws(() => {
        new BigNumber(1).egcd(-1 as any);
      }, /^Error: Assertion failed$/);
    });
  });

  describe("BN.max(a, b)", () => {
    it("should return maximum", () => {
      assert.equal(BigNumber.max(new BigNumber(3), new BigNumber(2)).toString(16), "3");
      assert.equal(BigNumber.max(new BigNumber(2), new BigNumber(3)).toString(16), "3");
      assert.equal(BigNumber.max(new BigNumber(2), new BigNumber(2)).toString(16), "2");
      assert.equal(BigNumber.max(new BigNumber(2), new BigNumber(-2)).toString(16), "2");
    });
  });

  describe("BN.min(a, b)", () => {
    it("should return minimum", () => {
      assert.equal(BigNumber.min(new BigNumber(3), new BigNumber(2)).toString(16), "2");
      assert.equal(BigNumber.min(new BigNumber(2), new BigNumber(3)).toString(16), "2");
      assert.equal(BigNumber.min(new BigNumber(2), new BigNumber(2)).toString(16), "2");
      assert.equal(BigNumber.min(new BigNumber(2), new BigNumber(-2)).toString(16), "-2");
    });
  });

  describe("BN.ineg", () => {
    it("shouldn't change sign for zero", () => {
      assert.equal(new BigNumber(0).ineg().toString(10), "0");
    });
  });
});